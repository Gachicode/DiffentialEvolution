...
int main(int argc, char* argv[]) {
    ...
    double F, CR;
    int iters = 100;
    F = 0.7; //We get num of parents by multiplication amount of genes by this number
    CR = 0.12;
    int NP = 80;

    std::ofstream myfile;
    myfile.open("optimal_parameters.csv");
    myfile << ";convergence; MAPE\n";
    ...
    for (size_t strategy = 1; strategy <= 3; strategy++)
    {
        ...
        //}
        myfile << "strategy " << strategy << ";" << total_string << '\n';
        std::cout << "NP=" << NP << " iteration with 3 strats is over\n";
        std::cout << total_string << "\n";
    }
    /*std::cout << total_string;
    myfile << total_string;*/
    std::cout << "NP=" << NP << " iteration with 3 strats is over\n";
